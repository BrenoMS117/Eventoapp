    ...
    <View style={styles.postCard}>
      <View style={styles.postLeft}>
        <View style={[styles.avatar, { backgroundColor: post.user.color }]}>
          <Text style={[styles.avatarText, { color: post.user.textColor }]}>{post.user.initials}</Text>
        </View>
        {post.verified && (
...
export default function FeedScreen() {
  const { feedPosts, events, addFeedPost, likePost, nearbyEventIds } = useApp();
  ...
  function submitPost() {
    if (!newText.trim() || !selectedEvent) return;
    addFeedPost({ eventId: selectedEvent.id, eventName: selectedEvent.name, text: newText.trim(), tag: `#${POST_TYPES.find(t => t.key === selectedType)?.label || 'Geral'}`, type: selectedType });
    setNewText(''); setComposing(false); setSelectedEvent(null); setSelectedType('geral');
  }
  ...
  return (
    <SafeAreaView style={styles.safe} edges={['top']}>
      <View style={styles.header}>
        ...
        <TouchableOpacity style={styles.composeBtn} onPress={() => setComposing(true)}>
      ...
      <KeyboardAvoidingView style={{ flex: 1 }} behavior={Platform.OS === 'ios' ? 'padding' : undefined}>
          ...
          <View style={styles.composer}>
            ...
            <ScrollView horizontal showsHorizontalScrollIndicator={false} style={{ marginBottom: 10 }}>
              {events.filter(e => e.isLive || nearbyEventIds.includes(e.id)).map(e => (
                <TouchableOpacity key={e.id} style={[styles.eventChip, selectedEvent?.id === e.id && styles.eventChipSelected]} onPress={() => setSelectedEvent(e)}>
                  <Text style={[styles.eventChipText, selectedEvent?.id === e.id && { color: '#fff' }]} numberOfLines={1}>{e.name}</Text>
                </TouchableOpacity>
              ))}
            </ScrollView>
            ...
            <View style={styles.typeRow}>
              {POST_TYPES.map(t => (
                <TouchableOpacity key={t.key} style={[styles.typeBtn, selectedType === t.key && styles.typeBtnActive]} onPress={() => setSelectedType(t.key)}>
                  <Text style={[styles.typeBtnText, selectedType === t.key && { color: '#fff' }]}>{t.icon} {t.label}</Text>
                </TouchableOpacity>
              ))}
            </View>
            <TextInput style={styles.composerInput} placeholder="O que está acontecendo agora?" placeholderTextColor={COLORS.textMuted} value={newText} onChangeText={setNewText} multiline autoFocus />
            <View style={styles.composerActions}>
              <TouchableOpacity style={styles.cancelBtn} onPress={() => setComposing(false)}>
                <Text style={styles.cancelBtnText}>Cancelar</Text>
              </TouchableOpacity>
              <TouchableOpacity style={[styles.postBtn, (!newText.trim() || !selectedEvent) && styles.postBtnDisabled]} onPress={submitPost} disabled={!newText.trim() || !selectedEvent}>
        ...
        <ScrollView style={styles.scroll} showsVerticalScrollIndicator={false}>
          {sortedPosts.map(post => <PostCard key={post.id} post={post} onLike={likePost} />)}
...
const styles = StyleSheet.create({
  ...
  composeBtn: { flexDirection: 'row', alignItems: 'center', gap: 6, backgroundColor: 'rgba(255,255,255,0.2)', paddingHorizontal: 12, paddingVertical: 7, borderRadius: RADIUS.full },
  composeBtnText: { color: '#fff', fontSize: 13, fontWeight: '600' },
  composer: { backgroundColor: COLORS.surface, borderBottomWidth: 0.5, borderBottomColor: COLORS.border, padding: 14 },
  composerLabel: { fontSize: 11, fontWeight: '600', color: COLORS.textSecondary, textTransform: 'uppercase', letterSpacing: 0.5, marginBottom: 8 },
  eventChip: { paddingHorizontal: 12, paddingVertical: 6, backgroundColor: COLORS.background, borderRadius: RADIUS.full, marginRight: 8, borderWidth: 0.5, borderColor: COLORS.border, maxWidth: 180 },
  ...
  typeBtn: { paddingHorizontal: 10, paddingVertical: 5, backgroundColor: COLORS.background, borderRadius: RADIUS.full, borderWidth: 0.5, borderColor: COLORS.border },
  typeBtnActive: { backgroundColor: COLORS.primary, borderColor: COLORS.primary },
  typeBtnText: { fontSize: 12, color: COLORS.text },
  composerInput: { backgroundColor: COLORS.background, borderRadius: RADIUS.md, padding: 12, fontSize: 14, color: COLORS.text, minHeight: 80, textAlignVertical: 'top', borderWidth: 0.5, borderColor: COLORS.border },
  composerActions: { flexDirection: 'row', justifyContent: 'flex-end', gap: 10, marginTop: 10 },
  ...
  postBtn: { backgroundColor: COLORS.primary, paddingHorizontal: 20, paddingVertical: 9, borderRadius: RADIUS.md },
  postBtnDisabled: { backgroundColor: COLORS.border },
  postBtnText: { color: '#fff', fontWeight: '600', fontSize: 14 },
  scroll: { flex: 1 },
  postCard: { backgroundColor: COLORS.surface, marginHorizontal: 12, marginTop: 10, borderRadius: RADIUS.lg, padding: 12, flexDirection: 'row', gap: 10, borderWidth: 0.5, borderColor: COLORS.border, ...SHADOW.sm },
  postLeft: { alignItems: 'center', position: 'relative' },
  avatar: { width: 36, height: 36, borderRadius: 18, justifyContent: 'center', alignItems: 'center' },
  avatarText: { fontSize: 13, fontWeight: '600' },
  verifiedDot: { width: 14, height: 14, borderRadius: 7, backgroundColor: COLORS.primary, justifyContent: 'center', alignItems: 'center', position: 'absolute', bottom: -2, right: -2, borderWidth: 1.5, borderColor: COLORS.surface },
  postBody: { flex: 1 },
  postHeader: { flexDirection: 'row', alignItems: 'center', gap: 6, marginBottom: 2 },
  ...
  postEventName: { fontSize: 11, color: COLORS.primary, fontWeight: '500', marginBottom: 4 },
