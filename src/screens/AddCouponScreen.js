...
const COUPON_TYPES = [
  { key: 'bebida', icon: '🍹', label: 'Bebida Grátis', desc: 'Uma bebida do cardápio', color: '#1D9E75' },
  { key: 'comida', icon: '🍕', label: 'Comida Grátis', desc: 'Um item do cardápio de comidas', color: '#D85A30' },
  { key: 'desconto', icon: '🎟', label: 'Desconto', desc: 'Porcentagem ou valor de desconto', color: '#BA7517' },
  { key: 'experiencia', icon: '🎧', label: 'Experiência', desc: 'Autógrafo, foto, acesso VIP etc.', color: '#533AB7' },
  { key: 'brinde', icon: '🎁', label: 'Brinde', desc: 'Item especial como lembrança', color: '#D85A30' },
  { key: 'acesso', icon: '🚪', label: 'Acesso Especial', desc: 'Área VIP, backstage etc.', color: '#0C447C' },
];
...
export default function AddCouponScreen({ navigation }) {
  ...
  function handleNext() {
    ...
    if (step === 2) {
      ...
      if (!qty || parseInt(qty) < 1) { Alert.alert('Adicione uma quantidade válida'); return; }
    }
        ...
        <ScrollView style={styles.scroll} showsVerticalScrollIndicator={false}>
          ...
          {step === 1 && (
            <View style={styles.stepContent}>
              ...
              <Text style={styles.stepSub}>Escolha o que você quer oferecer aos seus clientes</Text>
          ...
          {step === 2 && typeInfo && (
            <View style={styles.stepContent}>
              <View style={[styles.typePreview, { backgroundColor: typeInfo.color + '18', borderColor: typeInfo.color + '44' }]}>
                <Text style={styles.typePreviewIcon}>{typeInfo.icon}</Text>
                <Text style={[styles.typePreviewLabel, { color: typeInfo.color }]}>{typeInfo.label}</Text>
              </View>
              ...
              <View style={styles.qtyRow}>
                <TouchableOpacity
                  style={styles.qtyBtn}
                  onPress={() => setQty(q => String(Math.max(1, parseInt(q || '1') - 5)))}
                >
              ...
              <View style={styles.expiryGrid}>
                {EXPIRY_OPTIONS.map(opt => (
                  <TouchableOpacity
                    ...
                    style={[
                      styles.expiryBtn,
                      expiresAt === opt.key && { backgroundColor: COLORS.primary, borderColor: COLORS.primary }
                    ]}
                    onPress={() => setExpiresAt(opt.key)}
                  >
                    <Text style={[styles.expiryBtnText, expiresAt === opt.key && { color: '#fff' }]}>
                      {opt.label}
                    </Text>
                  </TouchableOpacity>
                ))}
          ...
          {step === 3 && typeInfo && (
            <View style={styles.stepContent}>
              ...
              <Text style={styles.stepSub}>É assim que os usuários vão ver</Text>

              {/* Preview card */}
              <View style={styles.previewCard}>
                <View style={[styles.previewBand, { backgroundColor: typeInfo.color }]}>
                  <Text style={styles.previewIcon}>{typeInfo.icon}</Text>
                  <View>
                    <Text style={styles.previewBandType}>{typeInfo.label}</Text>
                    <Text style={styles.previewBandEvent}>{businessStats.activeEventName}</Text>
                  </View>
                ...
                <View style={styles.previewBody}>
                  ...
                  <View style={styles.previewMeta}>
                    <Text style={styles.previewMetaText}>📍 {businessStats.venueName}</Text>
                    {expiresAt && <Text style={styles.previewMetaText}>⏱ Até {expiresAt}</Text>}
                  </View>
                  <View style={styles.previewQty}>
                    <Text style={styles.previewQtyText}>{qty}/{qty} disponíveis</Text>
                  </View>
                  <View style={styles.progressBg}>
                    <View style={[styles.progressFill2, { width: '100%', backgroundColor: typeInfo.color }]} />
                  </View>
                  <View style={styles.previewAction}>
                    <Text style={[styles.previewActionText, { color: typeInfo.color }]}>
              ...
              <View style={styles.notifInfo}>
                ...
                <Text style={styles.notifText}>
                  Usuários próximos serão notificados assim que o cupom for publicado
                </Text>
              </View>
              ...
              <View style={styles.summaryCard}>
                <View style={styles.summaryRow}>
                  ...
                  <Text style={styles.summaryValue}>{typeInfo.icon} {typeInfo.label}</Text>
                </View>
                <View style={styles.summaryRow}>
                ...
                <View style={styles.summaryRow}>
                  ...
                  <Text style={styles.summaryValue}>{expiresAt || 'Sem limite'}</Text>
                </View>
                <View style={styles.summaryRow}>
...
const styles = StyleSheet.create({
  ...
  progressFill: { height: '100%', backgroundColor: COLORS.primary, transition: '0.3s' },
  scroll: { flex: 1 },
  ...
  typeLabel: { fontSize: 13, fontWeight: '600', color: COLORS.text, marginBottom: 3 },
  ...
  fieldLabel: { fontSize: 12, fontWeight: '600', color: COLORS.textSecondary, marginBottom: 6, marginTop: 16 },
  input: {
  ...
  qtyInput: {
    ...
    backgroundColor: COLORS.surface,
    borderRadius: RADIUS.md, borderWidth: 0.5, borderColor: COLORS.border,
    textAlign: 'center', fontSize: 18, fontWeight: '700', color: COLORS.text,
  },
  ...
  previewBandType: { fontSize: 10, color: 'rgba(255,255,255,0.8)', fontWeight: '600', textTransform: 'uppercase' },
  ...
  previewTitle: { fontSize: 15, fontWeight: '600', color: COLORS.text, marginBottom: 4 },
  previewDesc: { fontSize: 13, color: COLORS.textSecondary, lineHeight: 18, marginBottom: 8 },
  ...
  progressBg: { height: 4, backgroundColor: COLORS.border, borderRadius: 2, overflow: 'hidden', marginBottom: 10 },
  progressFill2: { height: '100%', borderRadius: 2 },
  previewAction: {
    paddingTop: 10, borderTopWidth: 0.5, borderTopColor: COLORS.border,
  },
