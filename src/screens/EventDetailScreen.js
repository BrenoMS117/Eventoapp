...
export default function EventDetailScreen({ route, navigation }) {
  ...
  function submitRating() {
    ...
    setRatingSent(true);
    Alert.alert('Obrigado! 🙌', 'Sua avaliação foi enviada. Você ajudou outras pessoas a decidir!');
  }

  function handleRedeem(coupon) {
    if (!isNearby) {
      Alert.alert('Você precisa estar no local', 'Chegue perto do evento para resgatar cupons.');
      return;
    }
      ...
      [
        { text: 'Cancelar', style: 'cancel' },
        {
          ...
          onPress: () => {
            ...
            if (result.success) {
              Alert.alert('✅ Resgatado!', `Apresente ao atendente do ${coupon.venue}. Você receberá uma confirmação.`);
            } else {
    ...
    <SafeAreaView style={styles.safe}>
      ...
      <View style={[styles.header, { backgroundColor: event.gradient[0] }]}>
        ...
        <View style={{ flex: 1, marginLeft: 8 }}>
          ...
          <Text style={styles.headerSub}>{event.venue} · {event.address.split(' - ')[1]}</Text>
        </View>
        {isNearby && (
      ...
      <ScrollView style={styles.scroll} showsVerticalScrollIndicator={false}>
        ...
        {event.isLive && (
          <View style={[styles.liveBanner, { backgroundColor: event.gradient[0] + '22' }]}>
            ...
            <Text style={styles.liveBannerText}>
              <Text style={{ fontWeight: '700', color: event.gradient[0] }}>{event.checkedInCount.toLocaleString()} pessoas</Text>
              {' '}estão aqui agora
            </Text>
          </View>
        )}
        ...
        <View style={styles.statsGrid}>
          ...
          <StatCard
            label="Fila"
            value={event.queueMinutes > 0 ? `~${event.queueMinutes}min` : 'Sem fila'}
        ...
        {/* Accessibility notes */}
        {event.accessible && event.accessibilityNotes && (
          <View style={styles.accessCard}>
            <Ionicons name="accessibility" size={16} color={COLORS.primary} />
            <Text style={styles.accessText}>{event.accessibilityNotes}</Text>
          </View>
        )}

        {/* Quick rating */}
        <View style={styles.section}>
          <Text style={styles.sectionTitle}>Deixar avaliação rápida</Text>
          <View style={styles.ratingGrid}>
            {RATING_QUICK.map(r => (
              <TouchableOpacity
                ...
                onPress={() => !ratingSent && toggleRating(r.key)}
              >
          ...
          {ratingSent && (
            <View style={styles.ratingThanks}>
              ...
              <Text style={styles.ratingThanksText}>Avaliação enviada! Obrigado 🙌</Text>
            </View>
          )}
        </View>
        ...
        {coupons.length > 0 && (
          <View style={styles.section}>
            <View style={styles.sectionRow}>
              ...
              <Text style={styles.sectionCount}>{coupons.length} cupons</Text>
            </View>
            {coupons.map(coupon => {
              ...
              return (
                <TouchableOpacity
                  key={coupon.id}
                  style={[styles.couponCard, redeemed && styles.couponCardRedeemed]}
                  onPress={() => handleRedeem(coupon)}
                  activeOpacity={0.85}
                  disabled={isSoldOut}
                >
                  <View style={[styles.couponLeft, { backgroundColor: coupon.highlightColor + '22' }]}>
                    <Text style={styles.couponIcon}>{coupon.icon}</Text>
                  </View>
                  <View style={styles.couponBody}>
                    ...
                    <View style={styles.couponFooter}>
                      <Text style={styles.couponQty}>
                        {isSoldOut ? '0' : coupon.remainingQty}/{coupon.totalQty} restantes
                      </Text>
                      {!redeemed && !isSoldOut && (
                        <Text style={[styles.redeemBtn, { color: coupon.highlightColor }]}>
                          {isNearby ? 'Resgatar →' : '📍 Chegar ao local'}
                        </Text>
                      )}
                    </View>
                    {/* progress bar */}
...
const styles = StyleSheet.create({
  ...
  statsGrid: {
    ...
    paddingHorizontal: 12, gap: 8, paddingTop: 8,
  },
  statCard: {
    flex: 1, minWidth: '45%',
  ...
  accessText: { fontSize: 13, color: COLORS.primaryDark, flex: 1 },
  ...
  sectionCount: { fontSize: 12, color: COLORS.primary, fontWeight: '600' },
  ...
  couponCardRedeemed: { opacity: 0.7 },
  couponLeft: {
    width: 60, justifyContent: 'center', alignItems: 'center',
  },
  couponIcon: { fontSize: 26 },
  couponBody: { flex: 1, padding: 12 },
  couponTop: { flexDirection: 'row', alignItems: 'center', gap: 6, marginBottom: 4 },
  couponTypeBadge: {
  ...
  couponFooter: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', marginTop: 6 },
  couponQty: { fontSize: 11, color: COLORS.textMuted },
  redeemBtn: { fontSize: 13, fontWeight: '600' },
  qtyBar: {
