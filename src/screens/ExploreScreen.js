      ...
      <View style={styles.cardInfo}>
        <View style={styles.cardHeader}>
          <View style={{ flex: 1 }}>
            ...
            <Text style={styles.cardMeta}>
              ...
              {' '}{event.address.split(' - ')[1] || event.venue} · {event.distanceKm}km
            </Text>
          </View>
        ...
        <View style={styles.pillRow}>
          ...
          {event.queueMinutes > 0 && (
            <Pill icon="⏱" label={`Fila ~${event.queueMinutes}min`} color={COLORS.warningLight} text={COLORS.warning} />
          )}
          {event.accessible && (
            <Pill icon="♿" label="Acessível" color={COLORS.primaryLight} text={COLORS.primaryDark} />
          )}
          {event.nowPlaying && (
            <Pill icon="🎵" label={event.nowPlaying.slice(0, 18)} color={COLORS.purpleLight} text={COLORS.purple} />
          )}
        </View>
    ...
    <SafeAreaView style={styles.safe}>
      ...
      >
        {FILTERS.map(f => (
          <TouchableOpacity
            key={f}
            style={[styles.filterBtn, selectedEventFilter === f && styles.filterBtnActive]}
      ...
      <ScrollView style={styles.scroll} showsVerticalScrollIndicator={false}>
        {liveEvents.length > 0 && (
          <>
            ...
            {liveEvents.map(e => (
              <EventCard key={e.id} event={e} onPress={() => navigation.navigate('EventDetail', { eventId: e.id })} />
            ))}
          </>
        )}
...
const styles = StyleSheet.create({
  ...
  liveDot: {
    ...
    backgroundColor: COLORS.live, marginRight: 5,
  },
