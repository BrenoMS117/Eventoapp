...
export default function CouponDetailScreen({ route, navigation }) {
  ...
  function handleRedeem() {
    if (!isNearby) {
      Alert.alert('📍 Você precisa estar no local', `Chegue até ${coupon.venue} para resgatar este cupom.`);
      return;
    }
    Alert.alert(
      `Resgatar agora?`,
    ...
    <SafeAreaView style={styles.safe}>
      ...
      <ScrollView showsVerticalScrollIndicator={false}>
        ...
        <View style={[styles.hero, { backgroundColor: coupon.highlightColor }]}>
          ...
          <Text style={styles.heroVenue}>{coupon.venue} · {event?.address?.split(' - ')[1]}</Text>
        </View>
        ...
        <View style={styles.body}>
          ...
          {isRedeemed || showQR ? (
            ...
            <View style={styles.soldOutBanner}>
              ...
              <View>
                ...
                <Text style={styles.soldOutSub}>Todos os cupons foram resgatados</Text>
              </View>
            </View>
          ) : isNearby ? (
            <View style={styles.nearbyBanner}>
              ...
              <View>
                ...
                <Text style={styles.nearbySub}>Pode resgatar este cupom agora</Text>
              </View>
            </View>
          ) : (
          ...
          {/* QR Code (shown after redemption) */}
          {(isRedeemed || showQR) && (
          ...
          <View style={styles.card}>
            ...
            {coupon.expiresAt && (
              <View style={styles.expiryRow}>
                ...
                <Text style={styles.expiryText}>Válido até {coupon.expiresAt}</Text>
              </View>
            )}
          </View>
          ...
          <View style={styles.card}>
            ...
            <View style={styles.qtyRow}>
              <Text style={styles.qtyMain}>{coupon.remainingQty}</Text>
              <Text style={styles.qtySub}>/{coupon.totalQty} restantes</Text>
            </View>
            ...
            {coupon.remainingQty <= 5 && coupon.remainingQty > 0 && (
              <Text style={styles.lowStockText}>⚡ Últimas unidades! Resgate agora.</Text>
            )}
          </View>
          ...
          {!isRedeemed && !showQR && !isSoldOut && (
            <TouchableOpacity
              style={[
                styles.redeemBtn,
                { backgroundColor: isNearby ? coupon.highlightColor : COLORS.border }
              ]}
...
const styles = StyleSheet.create({
  ...
  qrWrapper: {
    borderWidth: 3, borderRadius: RADIUS.lg,
  ...
  qrCode: { fontSize: 12, fontWeight: '700', letterSpacing: 2, marginTop: 10, fontFamily: 'monospace' },
  ...
  expiryText: { fontSize: 13, color: COLORS.danger, fontWeight: '500' },
  qtyRow: { flexDirection: 'row', alignItems: 'baseline', gap: 4, marginBottom: 8 },
  qtyMain: { fontSize: 32, fontWeight: '700', color: COLORS.text },
  qtySub: { fontSize: 14, color: COLORS.textSecondary },
  progressBg: { height: 6, backgroundColor: COLORS.border, borderRadius: 3, overflow: 'hidden' },
  progressFill: { height: '100%', borderRadius: 3 },
  lowStockText: { fontSize: 12, color: COLORS.danger, fontWeight: '500', marginTop: 6 },
  redeemBtn: {
