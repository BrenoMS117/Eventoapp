export const validate = {
  ...
  minLength: (v, min, label = 'Campo') =>
    (v?.trim().length ?? 0) < min ? `${label} precisa ter ao menos ${min} caracteres.` : '',

  maxLength: (v, max, label = 'Campo') =>
    (v?.trim().length ?? 0) > max ? `${label} pode ter no máximo ${max} caracteres.` : '',

  time: (v) => {
    if (!v?.trim()) return 'Horário é obrigatório.';
    return /^([01]\d|2[0-3]):([0-5]\d)$/.test(v.trim()) ? '' : 'Formato: HH:MM (ex: 20:00).';
  },
