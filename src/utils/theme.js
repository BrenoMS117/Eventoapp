export const COLORS = {
  primary: '#0D9E75', primaryDark: '#085041', primaryLight: '#E1F5EE', primaryMid: '#1D9E75',
  accent: '#D85A30', accentLight: '#FAECE7',
  purple: '#533AB7', purpleLight: '#EEEDFE',
  amber: '#BA7517', amberLight: '#FAEEDA',
  background: '#F6F7F9', surface: '#FFFFFF', surfaceAlt: '#F0F2F5',
  text: '#1A1A2E', textSecondary: '#6B7280', textMuted: '#9CA3AF',
  border: '#E5E7EB', borderStrong: '#D1D5DB',
  success: '#059669', successLight: '#D1FAE5',
  warning: '#D97706', warningLight: '#FEF3C7',
  danger: '#DC2626', dangerLight: '#FEE2E2',
  live: '#EF4444', white: '#FFFFFF',
};
export const RADIUS = { sm: 8, md: 12, lg: 16, xl: 24, full: 999 };
export const SHADOW = {
  sm: { shadowColor: '#000', shadowOffset: { width: 0, height: 1 }, shadowOpacity: 0.06, shadowRadius: 4, elevation: 2 },
  md: { shadowColor: '#000', shadowOffset: { width: 0, height: 2 }, shadowOpacity: 0.10, shadowRadius: 8, elevation: 4 },
};